...
function run_genx_case_simple!(case::AbstractString, setup::Dict)
    ...
    if setup["ModelingToGenerateAlternatives"] == 1
        if setup["MonolithicMGA"] == 1
            println("Starting Monolithic Model to Generate Alternatives (MGA) Iterations")
            mono_mga(EP, case, setup, inputs, outputs_path)
        else
...
function run_genx_case_benders!(case::AbstractString, setup::Dict)
    ...
    master_sol_final, subop_sol, EP_master_final, LB_hist, UB_hist, cpu_time = benders_decomposition(setup,benders_dict);
    ...
    # #elapsed_time = @elapsed write_outputs(EP, outputs_path, setup, inputs)
    elapsed_time = @elapsed write_benders_results!(master_sol_final,subop_sol,case,mysetup,myinputs,myinputs_decomp)
    # println("Time elapsed for writing is")
    # println(elapsed_time)
    ...
    elapsed_time = @elapsed write_benders_output(LB_hist,UB_hist,cpu_time,outputs_path,setup,inputs,EP_master_final);
...
function run_genx_case_multistage_benders!(case::AbstractString, setup::Dict)
    ...
    master_sol_final, EP_master_final, LB_hist, UB_hist, cpu_time = benders_decomposition(setup,benders_dict);
    ...
    elseif setup["BD_Stab_Method"]=="l1_level_set" 
        outputs_path = joinpath(case, "Results_Benders_L1_LevelSet")
    elseif setup["BD_Stab_Method"]=="lsup_level_set" 
        outputs_path = joinpath(case, "Results_Benders_Lsup_LevelSet")
    elseif setup["BD_Stab_Method"]=="trust_region" 
        outputs_path = joinpath(case, "Results_Benders_TrustRegion")
    elseif setup["BD_Stab_Method"]=="in_out"
        outputs_path = joinpath(case, "Results_Benders_InOut")
    ...
    write_benders_output(LB_hist,UB_hist,cpu_time,outputs_path,setup,inputs_d,EP_master_final);
end
...
function run_benders_tests!(case::AbstractString, setup::Dict)
    ...
    for p in setup["BD_StabParamRange"]
        setup["BD_StabParam"] = p;
        setup["BD_Stab_Method"]="int_level_set";
        run_bd_exp(setup,benders_dict,inputs,case);

        setup["BD_StabParam"] = p;
        setup["BD_Stab_Method"]="int_level_set_dynamic";
        run_bd_exp(setup,benders_dict,inputs,case);
    end
...
function run_benders_multistage_tests!(case::AbstractString, setup::Dict)
    ...
    for p in setup["BD_StabParamRange"]
        ...
        #setup["BD_Stab_Method"]="int_level_set";
        run_bd_exp(setup,benders_dict,inputs_d,case);

        #setup["BD_Stab_Method"]="l2_level_set";
        #run_bd_exp(setup,benders_dict,inputs_d,case);

        #setup["BD_Stab_Method"]="l1_level_set";
        #run_bd_exp(setup,benders_dict,inputs_d,case);

        #setup["BD_Stab_Method"]="lsup_level_set";
        #run_bd_exp(setup,benders_dict,inputs_d,case);
        
    end

    #setup["BD_StabParam"] = 0;
...
function run_bd_exp(setup::Dict,benders_dict::Dict,inputs::Dict,case::AbstractString)
    
    stab_method = setup["BD_Stab_Method"];

    master_sol_final, EP_master_final,LB_hist, UB_hist, cpu_time = benders_decomposition(setup,benders_dict);

    ##### EP = get_optimal_model(master_sol_final,benders_dict["master_vars"],setup,inputs);

    if stab_method =="int_level_set_dynamic" 
        outputs_path = joinpath(case, "Results_Benders_Int_LevelSetDyn")
    elseif stab_method =="int_level_set" 
        outputs_path = joinpath(case, "Results_Benders_Int_LevelSet")
    elseif stab_method =="l2_level_set" 
        outputs_path = joinpath(case, "Results_Benders_L2_LevelSet")
    elseif stab_method =="l1_level_set" 
        outputs_path = joinpath(case, "Results_Benders_L1_LevelSet")
    elseif stab_method =="lsup_level_set" 
        outputs_path = joinpath(case, "Results_Benders_Lsup_LevelSet")
    elseif stab_method =="trust_region" 
        outputs_path = joinpath(case, "Results_Benders_TrustRegion")
    elseif stab_method =="in_out"
        outputs_path = joinpath(case, "Results_Benders_InOut")
    else
        outputs_path = joinpath(case, "Results_Benders")
    end
    ...
    ######## write_outputs(EP, outputs_path, setup, inputs);
    
    
    write_benders_output(LB_hist,UB_hist,cpu_time,outputs_path,setup,inputs,EP_master_final);
    
    reset_master_problem!(benders_dict)

end

function reset_master_problem!(benders_dict)

   master_cons = benders_dict["master_cons"];

   EP = benders_dict["EP_master"];

    for con in all_constraints(EP,include_variable_in_set_constraints=false)
